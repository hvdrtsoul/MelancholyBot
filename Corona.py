...
class Corona:
    ...
    def get_covid_stat(self):
        ...
        plt.bar(self.date_list, self.died_list, width=1, label='Умерло', color='dimgrey')
        plt.bar(self.date_list, self.cured_list, width=1, label='Вылечено', color='dodgerblue')
        plt.bar(self.date_list, self.active_list, width=1, label='Активных', color='firebrick')
        plt.legend(loc='upper left')
        plt.xticks(rotation=30)
        plt.suptitle('Россия - детальная статистика по коронавирусу')
        plt.ylim(0, 20000000)
